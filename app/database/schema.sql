...
CREATE INDEX net_worth_items_user_idx
...
CREATE TABLE todo_ai_assessments (
    id BIGSERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX todo_ai_assessments_user_idx
    ON todo_ai_assessments(user_id, classification, updated_at DESC);

CREATE TABLE todo_ai_work_sessions (
    id BIGSERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    todo_id INTEGER NOT NULL REFERENCES todos(id) ON DELETE CASCADE,
    content_fingerprint CHAR(64) NOT NULL,
...
CREATE TABLE todo_ai_work_messages (
    id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX todo_ai_work_messages_session_idx
